...
class ArchitechAditionalInfoCubit extends Cubit<ArchitechAditionalInfoState> {
  ...
  ArchitechAditionalInfoCubit(this.architechAditionalInfoRepository) : super(ArchitechAditionalInfoIntially());

  Future<void> createArchitechAditionalInfo(Map<String, dynamic> data) async {
    emit(ArchitechAditionalInfoLoading());
    try {
      final res = await architechAditionalInfoRepository.createArchitechAditionalInfo(data);
      if (res != null) {
        if(res.status==true){
